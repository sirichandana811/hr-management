...
export default function AttendanceHistoryPage() {
  ...
  const teacherId = searchParams.get("teacherId"); // get teacherId from URL
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // API should return { teacher: {...}, attendance: [...] }
      if (data.teacher) setTeacher(data.teacher);
      if (Array.isArray(data.attendance)) setHistory(data.attendance);
    };
  ...
  const filteredHistory = searchDate
    ? history.filter(
        (rec) => new Date(rec.date).toISOString().slice(0, 10) === searchDate
      )
    : history;

  // Count working days (unique days with at least one "Present")
